...
class ActionAbiTest : Spek({
    ...
    given("a transaction writer") {
        ...
        on("squish the action abi model") {
            ...
            val buyRamBytesAbi = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishBuyRamBytesBody(BuyRamBytesBody(BuyRamBytesArgs(
                "memtripissue",
                "memtripproxy",
                4096
            ))).toHex()
            ...
            val output = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishActionAbi(actionAbi).toHex()
