package io.golos.commun4j.chain.actions.abihex

import com.memtrip.eos.abi.writer.compression.CompressionType
import io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter
...
class SignedTransactionAbiTest : Spek({
    ...
    given("a transaction writer") {
        ...
        on("squish the transaction abi model") {
            ...
            val newAccountAbiHex = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishNewAccountBody(NewAccountBody(NewAccountArgs(
                "memtripissue",
                "memtripblock",
            ...
            val buyRamBytesAbiHex = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishBuyRamBytesBody(BuyRamBytesBody(BuyRamBytesArgs(
                "memtripissue",
                "memtripproxy",
                4096
            ))).toHex()

            val delegateBandwidthAbiHex = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishDelegateBandwidthBody(DelegateBandwidthBody(DelegateBandwidthArgs(
                "memtripissue",
                "memtripproxy",
                "51.2345 EOS",
                "171.2345 EOS",
            ...
            val transactionAbiHex = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishTransactionAbi(transactionAbi).toHex()
            ...
            val signedTransactionHex = io.golos.chain.actions.transaction.AbiBinaryGenTransactionWriter(CompressionType.NONE).squishSignedTransactionAbi(signedTransactionAbi).toHex()
