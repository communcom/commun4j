...
class ChainGetTableRowsTest : Spek({
    ...
    given("an Api") {
        ...
        on("v1/chain/get_table_rows") {

            val tableRows = chainApi.getTableRows(GetTableRows(
                "eosio",
                "eosio.token",
                "accounts",
                "",
            ...
            it("should return the table rows") {
                assertTrue(tableRows.isSuccessful)
                assertNotNull(tableRows.body())
            }
        }
    }
})